# src/backend/api_gateway/endpoints/ai_analysis.py
...
class IncidentStore:
    """In-memory incident storage (replace with DB in production)."""
    ...
    def save_incident(self, report: IncidentReport) -> bool:
        ...
        try:
            ...
            logger.info(f"Incident saved: {report.incident_id}")
            return True
    ...
    def list_incidents(self, severity_filter: Optional[str] = None) -> List[IncidentReport]:
...
async def get_incident_report_alias(incident_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "generated_at": incident.timestamp.isoformat(),
            "report_content": incident.dict()
        }
    except HTTPException:
        raise
    except Exception as e:
